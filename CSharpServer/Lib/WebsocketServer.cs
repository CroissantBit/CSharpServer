...
public class WebsocketServer
{
    public WebsocketServer(IPEndPoint endpoint)
    {
        ...
        options.Standards.RegisterRfc6455(factory => { factory.MessageExtensions.RegisterDeflateCompression(); });
        options.Transports.ConfigureTcp(transport =>
        {
    ...
    private static async Task AcceptWebSocketRequest(WebSocketListener webSocketListener, CancellationToken cancelToken)
    {
        await Task.Yield();
        while (!cancelToken.IsCancellationRequested)
        {
            try
            {
                var webSocket = await webSocketListener.AcceptWebSocketAsync(cancelToken).ConfigureAwait(false);
                if (webSocket == null)
                {
